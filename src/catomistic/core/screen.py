"""Screen surfaces for adsorption."""

from catomistic.config_models import Config
...
class MACECalculator(ASECalculator):
    """ASE calculator interface for MACE."""
    
    implemented_properties = ['energy', 'forces']
    
    def __init__(self, model_paths, device='cpu', dispersion=True, default_dtype='float32'):
        """Initialize MACE calculator.
        
        Args:
            model_paths: Path to MACE model(s)
        ...
        self.model = mace.load(model_paths, device=device, dtype=default_dtype)
        self.device = device
...
class Structure:
    ...
    def get_surface_structures(
        ...
        for cif_file in cif_files:
            ...
            bulk_atoms = read_cif(cif_file)
            bulk_struct = AseAtomsAdaptor.get_structure(bulk_atoms)

            for miller in miller_indices:
                # Create slab using pymatgen
                slab_gen = SlabGenerator(bulk_struct, miller, 20, 15)
                slabs = slab_gen.get_slabs()
                if not slabs:
                    logging.warning(f"No slabs generated for {cif_file} with Miller indices {miller}")
                    continue
                    
                slab = slabs[0]  # Take the first slab
                slab_atoms = AseAtomsAdaptor.get_atoms(slab)
                name = f"{Path(cif_file).stem}_{miller}"
                base_atoms = slab_atoms.repeat(surface_supercell)
                ...
                if metal_type == "bimetallic":
                    doped_surfaces = Structure._create_doped_surfaces(
                        base_atoms, name, miller, dopants, fractions,
